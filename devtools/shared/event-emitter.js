...
class EventEmitter {
  ...
  static once(target, type, listener) {
    let hasListener = !!listener;
    let isPredicateListener = isPredicate(listener);
    let isEventHandlerListener = isEventHandler(listener);
    ...
    return new Promise(resolve => {
      ...
      let newListener = (first, ...rest) => {
        if (hasListener) {
          if (isPredicateListener) {
            // if the `listener` given is a predicate, we want to remove the `newListener`
            // and resolve the `once`'s promise, only if the predicate returns `true`.

            let resolved = listener.when.call(target, first, ...rest);
            ...
            if (isEventHandlerListener) {
              ...
              // contextual object.
              listener[handler](listener, type, first, ...rest);
            } else {
              // Otherwise we'll just call it
              listener(target, first, ...rest);
            }
          ...
          // No `listener` was given, so we just remove the wrap listener and resolve
          // the promise.
          EventEmitter.off(target, type, newListener);
          resolve(first);
        }
