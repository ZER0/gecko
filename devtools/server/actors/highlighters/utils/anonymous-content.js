...
const { getCurrentZoom, getWindowDimensions,
  loadSheet } = require("devtools/shared/layout/utils");
...
const _update = Symbol("classList/update");
 ...
 */
class ClassList {
  constructor(className, onUpdate = () => {}) {
...
class CanvasFrameAnonymousContentHelper {
  constructor(highlighterEnv, nodeBuilder, prefix = "") {
  ...
  getElement(id) {
    let prefixedId = this.prefix + id;

    if (this.elements.has(prefixedId)) {
      return this.elements.get(prefixedId);
    }

    let className = this.getAttributeForElement(id, "class");
    let classList = new ClassList(className, () =>
  ...
  hide(id) {
    this.setAttributeForElement(id, "hidden", "true");
  }

  show(id) {
    this.removeAttributeForElement(id, "hidden");
  }
